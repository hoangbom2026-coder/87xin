...
NO_HU	SLOT	Nổ hũ / Slot (`/slots`)
CASINO	LIVE_CASINO	Casino Trực Tuyến
BAN_CA	FISHING	Bắn Cá
GAME_BAI	QIPAI	Game Bài
THE_THAO	SPORT_BOOK	Thể Thao
DA_GA	COCK_FIGHTING	Đá Gà
XO_SO	LOTTERY	 (`/lottery`, alias `lo-de`)
...
```
Hoặc gửi token trong query parameter:
```
?token=<token>
```
Update `getCurrentUser()` method trong `GameLaunchController.` để decode JWT token thực tế.
Testing
Chạy test script:
```bash
...
Kiểm tra mapping trong `GameCategoryMapping.`
...
├── app/
│   ├── constants/
│   │   └── GameCategoryMapping.      # Category & platform mapping
│   ├── controller/
│   │   └── api/
│   │       └── GameLaunchController. # Game launch endpoints
│   └── service/
│       └── GscPlusGameLaunchService. # GSC+ API integration
├── config/
...
Next Steps
✅ Implement proper JWT authentication
✅ Add game favorites/recent games
✅ Add game search functionality
✅ Implement game balance transfer
✅ Add game history tracking
✅ Implement free round support
✅ Add game maintenance status
